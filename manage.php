...
$instance = $DB->get_record('enrol', array('id'=>$enrolid, 'enrol'=>'gescof'), '*', MUST_EXIST);
$course = $DB->get_record('course', array('id'=>$instance->courseid), '*', MUST_EXIST);
$context = context_course::instance($course->id, MUST_EXIST);

require_login($course);
$canenrol = has_capability('enrol/gescof:enrol', $context);
$canunenrol = has_capability('enrol/gescof:unenrol', $context);
...
if (!$canenrol and !$canunenrol) {
    ...
    require_capability('enrol/gescof:enrol', $context);
    require_capability('enrol/gescof:unenrol', $context);
}
...
if (!$enrol_gescof = enrol_get_plugin('gescof')) {
    throw new coding_exception('Can not instantiate enrol_gescof');
}

$instancename = $enrol_gescof->get_instance_name($instance);

$PAGE->set_url('/enrol/gescof/manage.php', array('enrolid'=>$instance->id));
$PAGE->set_pagelayout('admin');
$PAGE->set_title($enrol_gescof->get_instance_name($instance));
...
$potentialuserselector = new \enrol_gescof\enrol_gescof_potential_participant('addselect', $options);
$potentialuserselector->viewfullnames = $viewfullnames;
$currentuserselector = new \enrol_gescof\enrol_gescof_current_participant('removeselect', $options);
...
if (empty($extendbase)) {
    if (!$extendbase = get_config('enrol_gescof', 'enrolstart')) {
...
$basemenu[4] = get_string('now', 'enrol_gescof') . ' (' . userdate($now, get_string('strftimedatetimeshort')) . ')';
...
if ($canenrol && optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoassign)) {
        foreach($userstoassign as $adduser) {
            ...
            $enrol_gescof->enrol_user($instance, $adduser->id, $roleid, $timestart, $timeend);
        }
...
if ($canunenrol && optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstounassign)) {
        foreach($userstounassign as $removeuser) {
            $enrol_gescof->unenrol_user($instance, $removeuser->id);
        }
    ...
    <tr>
      ...
      <td id="buttonscell">
          <div id="addcontrols">
              ...
              <p><label for="menuroleid"><?php print_string('assignrole', 'enrol_gescof') ?></label><br />
